/**
 * One-off: ensure Karim has a user + password in this Supabase project, then run
 * the seed-karim-workspace logic against whichever email he uses to log in.
 *
 * Usage:
 *   tsx --env-file=.env.local scripts/karim-setup.ts <email> <password>
 *
 * Both arguments REQUIRED — no hardcoded credentials.
 */
...
if (!EMAIL || !PASSWORD) {
  console.error("Usage: tsx scripts/karim-setup.ts <email> <password>");
  process.exit(2);
}

async function main() {
  const { data: { users } } = await sb.auth.admin.listUsers();
  console.log(`Existing users in project: ${users?.length ?? 0}`);
  ...
  if (!user) {
    ...
    console.log(`✓ Found user ${EMAIL} (${user.id}) — password updated`);
  }

  let { data: ws } = await sb.from("workspaces").select("id").eq("slug", SLUG).maybeSingle();
  if (ws) {
    console.log(`✓ Workspace ${SLUG} exists (${ws.id})`);
    await sb.from("workspaces").update({ owner_user_id: user.id }).eq("id", ws.id);
    await sb
    ...
    const { data: createdWs } = await sb
      ...
      .single();
    ws = createdWs;
    console.log(`✓ Created workspace ${SLUG} (${ws!.id})`);
  ...
  console.log(`\n========================================`);
  console.log(`✓ Ready. Log in at: http://localhost:3000/login`);
